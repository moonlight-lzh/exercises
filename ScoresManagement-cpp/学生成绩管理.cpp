...
public:
    ...
    void statistic_grades()
    {
        ...
        vector<vector<int> > grades(n);
        for (size_t i = 0; i < n; i++)
        {
            grades[i].resize(5, 0);     // 每种课程5个等级
        }

        while (node)
        {
            for (size_t i = 0; i < n; i++)      // n个课程
            {
                grades[i][get_grades(node->student.scores[i])]++;
            }
            student_size++;
            node = node->next;
        }
        ...
        for (size_t i = 0; i < n; i++)
        {
            ...
            {
                percent = (double)grades[i][j] / student_size * 100;
                printf("\t%c:\t%d\t%.2lf%%\n", 'A' + j, grades[i][j], percent);
            }
        }
    }
};
...
int show_statistic_menu()
{
    ...
    switch (choice)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            manage->statistic_grades();     // 按照学生成绩等级统计
            wait_to_continue();
            break;
        case 0:
            break;
